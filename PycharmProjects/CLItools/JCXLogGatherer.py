...
def collectLogs():
    installationName = input("Installation name (i.e. jive-noc-jcx): ")
    ...
    downloadPath = input("Path where would you like the logs put (i.e. ~/Downloads/logs/): ")
    collecting = True
    ...
    while collecting:
        ...
        execPrefix = "kubectl exec -c webapp --context=jcx-prod-us-east -n " + namespaceName + " -it " + podName + ' -- bash -c "'
        execCommand = ''
        execSuffix = '"'
        ...
        print("\n------------------------------   Downloading logs ------------------------------ ")
        os.system("kubectl cp -c webapp --context=jcx-prod-us-east -n " + namespaceName + " " + podName + ":/var/log/jive/" + archiveName + " " + downloadPath + archiveName)
